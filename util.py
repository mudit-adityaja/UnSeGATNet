...
cmap = 'tab20'
def save_or_show(arr, filename, dir1, save=False):
    if save:
        Image.fromarray(arr[0]).convert('RGB').save(dir1+ '/' + filename + '_org' + '.png')
        Image.fromarray(arr[1]).convert('RGB').save(dir1+ '/' + filename + '_mask' + '.png')
        Image.fromarray(arr[2]).convert('RGB').save(dir1+ '/' + filename + '_fused' + '.png')
        if len(arr)>3:
            Image.fromarray(arr[3]).convert('RGB').save(dir1+ '/' + filename + '_true' + '.png')
        # plt.imsave(dir1 + filename + '_org' + '.png', arr[0], cmap=cmap)
        # plt.imsave(dir1 + filename + '_mask' + '.png', arr[1], cmap=cmap)
        # plt.imsave(dir1 + filename + '_fused' + '.png', arr[2], cmap=cmap)
    else:
...
def knn_affinity(image, n_neighbors=[20, 10], distance_weights=[2.0, 0.1]):
    ...
    image = rgb2hsv(image) # Convert rgb to hsv
    h, w = image.shape[:2]
    ...
    # print(f'w={w},h={h},x shape={x.shape}, yshape = {y.shape}, rshape={r.shape},gshape={g.shape},bshape={b.shape}')

    i, j = [], []

    for k, distance_weight in zip(n_neighbors, distance_weights):
        f = np.stack(
            [np.cos(r),np.sin(r), g, 0.5*b, distance_weight * x, distance_weight * y],
            axis=1,
...
def load_data_img1(chosen_dir, image_size):
    """
    Load image to model (Resize, To tensor, normalize)
    @param chosen_dir: Directory for loaded image
    @param image_size: Output size for image
